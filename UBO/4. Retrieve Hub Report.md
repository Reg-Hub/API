...
Once your order is in status "Complete" you may retreive your hub report. We provided a nice PDF that packages all relevant information from the hub report tree into one place. You may retreive this report at the following endpoint.
...
If you wish to or are required to provide a version for your portal or construct your own document, this is still possible. You may use the same endpoint as above with the accept header set to application/json.
...
We also provide a conveniency endpoint with the ownership summary for the hub report.

```
GET {root}/api/v1/Orders/{orderID}/HubReport/OwnershipSummary
```

Where the order ID is any order that exists in the hub report. This will provide a high level ownership summary of all discovered shareholders in the UBO tree, including any shareholders discovered through recursion (see step 5). An example response looks like the following:
...
// TODO: Finish section when PEPs and Sanctions are exposed publicly.
